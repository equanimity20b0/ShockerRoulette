...
class ShockerClient(HardwareManager):
    def __init__(self, config: dict, config_path: str):
        ...
        self.running = True
        self.roulette_start_intensity = 10
        self.roulette_max_intensity = 50
        self.roulette_current_intensity = 10
    ...
    async def receive_loop(self):
        ...
        try:
            async for message in self.websocket:
                ...
                # Check for stakes properties in any payload
                if "roulette_start_intensity" in data:
                    self.roulette_start_intensity = data["roulette_start_intensity"]
                if "roulette_max_intensity" in data:
                    self.roulette_max_intensity = data["roulette_max_intensity"]
                if "roulette_current_intensity" in data:
                    self.roulette_current_intensity = data["roulette_current_intensity"]
                ...
                if event == "welcome":
                    ...
                    if not self.voice_started:
                        # Set initial microphone device from config
                        mic_id = self.config.get("mic_device_id")
                        if mic_id is not None:
                            try:
                                VoiceRecog.active_device_id = int(mic_id)
                            except Exception:
                                VoiceRecog.active_device_id = None
                    ...
                    vibrate_intensity = data.get("vibrate_intensity", 100)
                    
                    self.add_log("server", "Roulette triggered! Spinning the wheel...")
                    print(f"\n{bcolors.WARNING}🎯 ROULETTE TRIGGERED! Spinning the wheel...{bcolors.ENDC}")
                    
                    asyncio.create_task(self.run_roulette_sequence(
                        ticking_sequence, tick_delay_ms, vibrate_intensity
                    ))
    ...
    async def run_roulette_sequence(self, sequence, delay_ms, vibrate_intensity):
        """Simulate the physical ticking roulette by vibrating player collars sequentially."""
        ...
        print(f"\n{bcolors.HEADER}🎯 Spin finished! Awaiting selection result...{bcolors.ENDC}")

    def update_config(self, new_config: dict):
        """Update configurations via Web UI dynamically."""
        old_server_ip = self.config.get("server_ip")
        old_mic_id = self.config.get("mic_device_id")

        # Strict limit validation for user physical hardware safety
        if "max_intensity" in new_config:
            try:
                val = int(new_config["max_intensity"])
                if not (1 <= val <= 100):
                    raise ValueError("max_intensity must be between 1 and 100")
            except (TypeError, ValueError) as e:
                raise ValueError(f"Invalid max_intensity: {e}")

        if "max_duration_ms" in new_config:
            try:
                val = int(new_config["max_duration_ms"])
                if not (100 <= val <= 15000):
                    raise ValueError("max_duration_ms must be between 100 and 15000 (ms)")
            except (TypeError, ValueError) as e:
                raise ValueError(f"Invalid max_duration_ms: {e}")
        ...
        # Check if mic_device_id changed to trigger dynamic hot-swap
        new_mic_id = self.config.get("mic_device_id")
        if new_mic_id != old_mic_id:
            if new_mic_id is not None:
                try:
                    VoiceRecog.active_device_id = int(new_mic_id)
                    self.add_log("server", f"Microphone input swapped to device ID {new_mic_id}")
                except Exception:
                    VoiceRecog.active_device_id = None
            else:
                VoiceRecog.active_device_id = None
